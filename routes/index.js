...
function sendSong(req, res) {
    ...
    var ext = req.params.id.substring(req.params.id.indexOf('.') + 1);
    app.db.songs.findOne({_id: id}, function (err, song) {
        if (err || !song) {
            res.status(404).send();
        } else {
            var requestedSong = path.join(app.get('config').music_dir, song.location);
            console.log('Play ' + id + ' with ext ' + ext);
            lib_func.getPlayableMediaFile(id, requestedSong, ext, res);
        }
    });
}
