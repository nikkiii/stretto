...
var supportedMedia = ['.mp3', '.wav', '.flac'];
...
function getPlayableMediaFile(id, source, ext, res) {
    console.log('Source: ' + source + ' for ' + ext + ' (' + path.extname(source) + ')');
    var originalExt = path.extname(source).substring(1);
    if (ext == originalExt) {
        ...
        mkdirp(folder, function () {
            ...
            try {
                ...
                res.status(200);
                _convertToMp3(id, source, destination, res, function () {
...
function _convertToMp3(id, source, destination, res, done) {
    ffmpeg(source)
        .noVideo()
        .audioCodec('libmp3lame')
        .audioQuality(1)
        ...
        .on('progress', function (progress) {
            broadcast('conversion_progress', {id: id, source: source, progress: progress.percent});
        ...
        .on('end', function() {
            ...
            broadcast('conversion_done', {id: id, source: source, progress: 100});
        }).run();
}
