...
socket.on('conversion_progress', function (data) {
    if (player.playing_id == localStorage.getItem('last_playing_id')) {
        // Slider background update
        player.conversionStatus(data.progress);
    } else {
        player.conversionDone();
    }
});

socket.on('conversion_done', function (data) {
    if (player.playing_id == localStorage.getItem('last_playing_id')) {
        // Slider background update
        player.conversionDone();
    }
