...
function conversionDone() {
        ...
        $('#bottom_bar').css('height', '61px');
        $('#scrub_bar_slider').css('bottom', '55px');
}
...
function PlayState() {
    ...
    this.conversionStatus = function (progress) {
        var $progress = $('#conversionProgress');
        ...
        $('#bottom_bar').css('height', '81px');
        $('#scrub_bar_slider').css('bottom', '75px');
    };
    ...
    this.PlayMethodAbstracter = new (function () {
        ...
        this.$audio_elem = $(this.audio_elem);
        ...
        this.playTrack = function (songInfo) {
            if (!songInfo.attributes.is_youtube) {
                ...
                this.last_info = songInfo;

                var originalExt = songInfo.attributes.location.substring(songInfo.attributes.location.lastIndexOf('.') + 1);

                // load in the new audio track
                this.audio_elem.pause();

                this.$audio_elem.children('source').remove();
                ...
                var $srcs = [
                    $('<source/>').attr('src', 'songs/' + songInfo.attributes._id + '.mp3').attr('type', 'audio/mpeg')
                ];

                if (originalExt != 'mp3') {
                    $srcs.push($('<source>').attr('src', 'songs/' + songInfo.attributes._id + '.' + originalExt).attr('type', audioTypes[originalExt]));
                }

                this.$audio_elem.append($srcs);

                this.srcElem.attr('src', 'songs/' + songInfo.attributes._id + '.' + originalExt);
        ...
        this.getDuration = function () {
            ...
            } else if (this.audio_elem.duration && isFinite(this.audio_elem.duration)) {
                return this.audio_elem.duration;
            } else if (this.last_info.attributes.duration) {
                return this.last_info.attributes.duration;
            }
        };
